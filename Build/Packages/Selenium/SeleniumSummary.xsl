  ...
  <xsl:output method="xml" omit-xml-declaration="yes"/>
      ...
      <seleniumsummary>
        ...
        <xsl:choose>
          <xsl:when test="$junit.case.count = 0">
            <tr>
              <td colspan="2" class="section-data">No  Web Tests Run</td>
            </tr>
            <tr>
              <td colspan="2" class="section-error">This project doesn't have any web tests</td>
            </tr>
          </xsl:when>

          <xsl:when test="$junit.failure.count + $junit.error.count = 0">
            <tr>
              <td colspan="2" class="section-data">All Web Tests Passed</td>
            </tr>
          </xsl:when>
        </xsl:choose>
        ...
        <xsl:if test="$junit.failure.count + $junit.error.count > 0">
          <tr>
            <td class="sectionheader-container-error" colspan="2">
              Web Test Failure and Error Details (<xsl:value-of select="$junit.failure.count + $junit.error.count"/>)
            </td>
          </tr>
    ...
    <xsl:template name="junittestdetail">
      ...
      <xsl:for-each select="$detailnodes">
        ...
        <tr><td class="section-data">WebTest:</td><td class="section-data"><xsl:value-of select="@name"/></td></tr>
