  ...
  <xsl:template match="/">
    ...
    <integration>
      ...
      <xsl:call-template name="AddStatistic">
        ...
        <xsl:with-param name="StatisticValue" select="($CCNetLog)//TargetProcess/Iteration/@name[position()=last()]"/>
      </xsl:call-template>
      <xsl:call-template name="AddStatistic">
        <xsl:with-param name="StatisticName" select="'IterationStartDate'"/>
        <xsl:with-param name="StatisticValue" select="($CCNetLog)//TargetProcess/Iteration/@startdate[position()=last()]"/>
      </xsl:call-template>
      <xsl:call-template name="AddStatistic">
        <xsl:with-param name="StatisticName" select="'IterationEndDate'"/>
        <xsl:with-param name="StatisticValue" select="($CCNetLog)//TargetProcess/Iteration/@enddate[position()=last()]"/>
      </xsl:call-template>
      
      <xsl:call-template name="AddStatistic">
        <xsl:with-param name="StatisticName" select="'Executed'"/>
        <xsl:with-param name="StatisticValue" select="boolean(//result)"/>
      </xsl:call-template>

      <xsl:call-template name="AddStatistic">
        <xsl:with-param name="StatisticName" select="'Test Count'"/>
        <xsl:with-param name="StatisticValue" select="count(//result)"/>
      </xsl:call-template>

      <xsl:call-template name="AddStatistic">
        <xsl:with-param name="StatisticName" select="'Test Correct Count'"/>
        <xsl:with-param name="StatisticValue" select="count(//result/counts[right[text() &gt; 0] and not(ignores[text() &gt; 0]) and not(wrong[text() &gt; 0]) and not(exceptions[text() &gt; 0])])"/>
      </xsl:call-template>
      
      <xsl:call-template name="AddStatistic">
        <xsl:with-param name="StatisticName" select="'Test Failure Count'"/>
        <xsl:with-param name="StatisticValue" select="count(//result/counts/wrong[text() &gt; 0])"/>
      </xsl:call-template>

      <xsl:call-template name="AddStatistic">
        <xsl:with-param name="StatisticName" select="'Test Exception Count'"/>
        <xsl:with-param name="StatisticValue" select="count(//result/counts/exceptions[text() &gt; 0])"/>
      </xsl:call-template>

      <xsl:call-template name="AddStatistic">
        <xsl:with-param name="StatisticName" select="'Correct Validation Count'"/>
        <xsl:with-param name="StatisticValue" select="sum(//result/counts/right)"/>
      </xsl:call-template>

      <xsl:call-template name="AddStatistic">
        <xsl:with-param name="StatisticName" select="'Failed Validation Count'"/>
        <xsl:with-param name="StatisticValue" select="sum(//result/counts/wrong)"/>
      </xsl:call-template>

      <xsl:call-template name="AddStatistic">
        <xsl:with-param name="StatisticName" select="'Exception Count'"/>
        <xsl:with-param name="StatisticValue" select="sum(//result/counts/exceptions)"/>
      </xsl:call-template>
