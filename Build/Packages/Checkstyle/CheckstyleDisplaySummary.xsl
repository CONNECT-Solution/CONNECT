...
<xsl:stylesheet
  ...
  <xsl:template match="/">
    <xsl:variable name="stuff" select="//checkstylesummary" />
          ...
          <td height="42" class="sectionheader-container" colSpan="2">
            <a style="TEXT-DECORATION:NONE; color:403F8D;" onmouseover="this.style.color='#7bcf15'" onmouseout="this.style.color='#403F8D'">
              ...
              <div class="sectionheader-text">
                Checkstyle Summary (<xsl:value-of select="$stuff/@percentduplication" /> Duplication)
              </div>
            </a>
          </td>
        </tr>
        <xsl:copy-of select="//checkstylesummary"/>
      </table>
    </xsl:if>
  </xsl:template>
</xsl:stylesheet>
