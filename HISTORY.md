...
Release 4.6 continues to build on the functionality and architecture introduced in Release 4.0 through additional features, selected improvements, and bug fixes.This release includes enhancements to audit logging along with compliance to the NwHIN/ATNA audit message requirements for Patient Discovery (PD), Query for Documents (QD), Retrieve Documents (RD), Document Submission (DS) and SOAP/X12 services. These audit events can be searched and viewed through the System Administration Module to help with monitoring and troubleshooting transactions and providing additional insight for reporting.

With Release 4.6, CONNECT is now an eHealth Exchange Validated Product. This recognition should give the CONNECT community, including federal agencies as well as potential new adopters, a very high confidence in CONNECT in that it is specification compliant, interoperable and has been successfully validated for the eHealth Exchange. 
...
However, a couple of issues were discovered in some adopter environments. These were attributed to jar files that was missing. The following CXF jars were added to 4.6 code base and packaged within the CONNECT ear in Release 4.6 and updates were made to the pom files as well.
...
Release 4.5 continues to build on the functionality and architecture introduced in Release 4.0 through further features, selected improvements, and bug fixes, as well as incorporating feedback from the community.  This is an exciting release enhancing the features supported by CONNECT while also reducing the operational and configuration level of effort.  In the past couple of releases, CONNECT has an operational dashboard, configuration of the Direct Health Information Service Provider, and ping utility via the System Administration Module. In Release 4.4, CONNECT implemented the NwHIN CAQH CORE X12 Document Submission specifications enabling CMS and other providers the ability to exchange X12 payloads using both Real Time and Generic Batch transactions.

With CONNECT 4.5, the X12 service has been enhanced to audit X12 transactions utilizing industry standards like ATNA and CAQH CORE X12 specifications. Payload metadata and ATNA compliant fields are audited for each transaction. Adopters will have the option of turning auditing off or turning on which enables audit logging to the database. 

The System Administrative Module is created with the goal to help adopters to accelerate their implementation time frames and increase adoption and partner expansion. This module has been enhanced this release to include the ability to view and configure gateway configuration files. Both gateway.properties and adapter.properties can be configured through the module, without having to access the property files at the system level. This allows managers with limited technical system knowledge to update configuration properties simplifying gateway setup and making it easier for adopters to manage the gateway instance. In the previous release, CONNECT 4.4.1 had introduced the Connection Management function to display endpoints from the UDDI Connection files along with the ability to "Ping" trading partners to determine their gateway status to help with troubleshooting connectivity, this enhancement will also be available with 4.5. Also included is a new test tool that will help administrators to test PD, QD and RD outbound requests from the gateway in pass-through mode has also been added to the System Administrative module. These enhancements provide administrators of the system to do preliminary testing at the gateway to identify connectivity and service issues sooner rather than later.  This testing tool is also part of the enhancement and migration of functionality from the legacy Universal Client into the System Administration Module.

4.5 also introduced a new feature to better manage transactions and exchange traffic with partners.  The entity interface has been updated to include a new parameter to the assertion block (optional element) to control web service timeout for any transaction. This will enable adopters to configure timeouts from the client adapter for individual messages and would allow them to account for larger payloads, slow responses from particular trading partners or for particular messages, and any other connectivity issues that might alter expected connection time for a given gateway to gateway message.  This is the initial roll out of this feature and as the CONNECT Team works with implementers it will be further enhanced and modified to provide even more control and value.

Release 4.5 also includes bug fixes and other improvements to the product making it more robust and secure. During development of 4.5 the Product Team worked closely with external testing tools and groups to further assist adopters with a smoother process testing and with their exchange partners. Security scans were run on the deployable and packaged source code and all findings of Critical, High and Medium priority issues were addressed. 
...
Release 4.4 continues to build on the functionality and architecture introduced in Release 4.0 through further features, selected improvements, and bug fixes, as well as incorporating feedback from the community.  This is an exciting release introducing new functionality extending the use cases supported by CONNECT while also reducing the operational and configuration level of effort.  
...
As mentioned above, CONNECT 4.4. introduces a new Graphical User Interface (GUI) for Administrative functions. The System Administration Module has been developed as a simple and easy to configure the application with the goal to help adopters to accelerate their implementation time frames and increase adoption and partner expansion. This module will continued to evolve and support adopters with important technical abilities to manage the CONNECT instance. The base framework, user management, usage monitoring and Direct configuration was completed in Release 4.4 as the initial phase.  

Release 4.4 broadens the supported message transports and use cases that can be used for healthcare data exchange by introducing the NwHIN CAQH Core X12 Document Submission service. X12 transactions are widely utilized in the payer/provider world.  This feature allows adopters like CMS and its trading partners and the broader community to now exchange  X12 payloads utilizing NwHIN specifications in CONNECT, furthering the return on investment from a single implementation.

Release 4.4 also includes bug fixes and other improvements to the product making it more robust and secure. During development of 4.4 the Product Team worked closely with external testing tools and groups to further assist adopters with a smoother process testing and with their exchange partners. Security scans were run on the deployable and packaged source code and all findings of Critical, High and Medium priority issues were addressed. 

Release 4.4 was tested with previous versions of CONNECT and meets backwards compatibility requirements with versions 4.3.2, 4.2.2.2 and 3.3.1.3.  Release 4.4 has been tested against all test cases for eHealth Exchange automated participant testing.  Work continues with Healtheway on manual checklist specification requirements. Release 4.4 was also tested against the NIST test environment for the XDR test cases as well as the Direct transport test cases. 
...
Release 4.3 continues to build on the functionality introduced in Release 4.0 through further features, selected improvements, and bug fixes, as well as incorporating feedback from the community.  

CONNECT 4.0 through 4.2 provided the community with a robust enterprise-ready gateway with higher throughput  and large payload processing capabilities, supporting the future exchange needs of our adopters. The incorporation of Direct into the product made CONNECT a true platform supporting the different transports necessary for Meaningful Use of health data systems and that are utilized in the healthcare ecosystem for health data exchange. To support the broader community and their preferred deployment environment, the CONNECT technology stack was changed to support application servers like WebSphere, WebLogic, and JBoss, in addition to the GlassFish application server.

A lot was accomplished in Release 4.0 through 4.2 to support expanding federal, commercial, and regional HIE needs with regards to performance, deployment options, and supporting different transports. As demands for immediate production readiness to participate in the aforementioned exchange areas continue to be a key factor for adopters and a key measure for their return-on-investment, the CONNECT Team has focused on ensuring that CONNECT 4.3 and future releases deliver with a high level of production readiness. Therefore, enhancements for CONNECT have focused on ensuring security requirements are met, as well as meeting the requirements for reduced deployment timelines and the ability to meet industry certifications. In support of these goals, testing enhancements and automation were a priority during Release 4.3, both of which will enable a smoother migration towards a more open contribution model with the CONNECT community. A solid testing infrastructure and process to support product validation is necessary, especially to understand how any new features impact existing functionality. 
...
* Modifying xsi:type values to be CE instead of hl7:CE in the PurposeOfUse and Role attributes(CONN-875) and including the appropriate namespace xsi
...
ONNECT 4.2 addresses a few bugs that are critical to interoperability, security and specification compliance, with a focus on Healtheway participant certification testing.  

As part of the Java 6 support validation, CONNECT 4.2 was tested on WAS 7 application server.  The WAS 7 environment will no longer be available to the CONNECT team and as a result this will be the last version of CONNECT to tested on WAS 7 application server. Future versions of CONNECT will be tested using later versions of the WebSphere application server.
