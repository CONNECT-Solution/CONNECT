...
public class DocumentQueryClient {
    ...
    public List<DocumentInformation> retrieveDocumentsInformation(PatientSearchData patientSearchData, Date creationFromDate, Date creationToDate) {
        ...
        if (retryCount > 0 && retryDelay > 0 && exceptionText != null && !exceptionText.equalsIgnoreCase("")) {
            ...
            while (i <= retryCount) {
                try {
                    response = port.respondingGatewayCrossGatewayQuery(request);
                    break;
                    ...
                    if (flag == 1) {
                        ...
                        try {
                            ...
                            log.error("Thread Got Interrupted while waiting on EntityDocQuery call :" + iEx);
                        } catch (IllegalArgumentException iaEx) {
                            log.error("Thread Got Interrupted while waiting on EntityDocQuery call :" + iaEx);
                        }
                        retryDelay = retryDelay + retryDelay; //This is a requirement from Customer
                    } else {
                        log.error("Unable to call EntityDocQuery Webservice due to  : " + e);
                        throw e;
                    }
                }
            }
            ...
            if (i > retryCount) {
                log.error("Unable to call EntityDocQuery Webservice due to  : " + catchExp);
                throw catchExp;
            }
