...
//            oDocRepositoryPort = oDocRepService.getDocumentRepositoryPortSoap();
    ...
    private void saveCPPDoc(String sPrefDoc, PatientPreferencesType oPtPref, String sUniqueDocumentId, String sMimeType)
        ...
        RegistryResponseType oRegistryResponse = null;
	...
	int retryDelay = gov.hhs.fha.nhinc.webserviceproxy.WebServiceProxyHelper.getInstance().getRetryDelay();
        ...
        if (retryCount > 0 && retryDelay > 0 && exceptionText != null && !exceptionText.equalsIgnoreCase("")) {
            ...
            while (i <= retryCount) {
                try {
                    oRegistryResponse = oDocRepositoryPort.documentRepositoryProvideAndRegisterDocumentSetB(oRequest);
                    break;
                    ...
                    if (flag == 1) {
                        ...
                        try {
                            ...
                            log.error("Thread Got Interrupted while waiting on PatientConsentManager.DocumentRegistryService call :" + iEx);
                        } catch (IllegalArgumentException iaEx) {
                            log.error("Thread Got Interrupted while waiting on PatientConsentManager.DocumentRegistryService call :" + iaEx);
                        }
                        retryDelay = retryDelay + retryDelay; //This is a requirement from Customer
                    } else {
                        log.error("Unable to call PatientConsentManager.DocumentRegistryService Webservice due to  : " + e);
                        throw e;
                    }
                }
            }
            ...
            if (i > retryCount) {
                log.error("Unable to call PatientConsentManager.DocumentRegistryService Webservice due to  : " + catchExp);
                throw catchExp;
            }
    ...
    private void retrieveCPPDoc(DocumentRequest oDocRequest, CPPDocumentInfo oCPPDocInfo)
        ...
        if (retryCount > 0 && retryDelay > 0 && exceptionText != null && !exceptionText.equalsIgnoreCase("")) {
                ...
                try {
                    ...
                    if (flag == 1) {
                        ...
                        try {
                            ...
                            log.error("Thread Got Interrupted while waiting on PatientConsentManager.DocumentRepositoryService call :" + iEx);
                        } catch (IllegalArgumentException iaEx) {
                            log.error("Thread Got Interrupted while waiting on PatientConsentManager.DocumentRepositoryService call :" + iaEx);
                        }
                        retryDelay = retryDelay + retryDelay; //This is a requirement from Customer
                    } else {
                        log.error("Unable to call PatientConsentManager.DocumentRepositoryService Webservice due to  : " + e);
                        throw e;
                    }
                }
            }
            ...
            if (i > retryCount) {
                log.error("Unable to call PatientConsentManager.DocumentRepositoryService Webservice due to  : " + catchExp);
                throw catchExp;
            }
