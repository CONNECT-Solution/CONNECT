...
<!-- Title: CDA XSL StyleSheet Original Filename: cda.xsl Version: 3.0 Revision History: 08/12/08 Jingdong Li updated Revision 
    History: 12/11/09 KH updated Revision History: 03/30/10 Jingdong Li updated. Revision History: 08/25/10 Jingdong Li updated 
    Revision History: 09/17/10 Jingdong Li updated Revision History: 01/05/11 Jingdong Li updated Specification: ANSI/HL7 CDAR2 
    The current version and documentation are available at http://www.lantanagroup.com/resources/tools/. We welcome feedback 
    and contributions to tools@lantanagroup.com The stylesheet is the cumulative work of several developers; the most significant 
    prior milestones were the foundation work from HL7 Germany and Finland (Tyylitiedosto) and HL7 US (Calvin Beebe), and the 
    presentation approach from Tony Schaller, medshare GmbH provided at IHIC 2009. -->
<!-- LICENSE INFORMATION Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except 
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:n1="urn:hl7-org:v3"
    ...
    <xsl:template match="n1:ClinicalDocument">
        <html>
            <head>
                <xsl:comment>
                    Do NOT edit this HTML directly: it was generated via an XSLT transformation from a CDA Release 2 XML
                    document.
                </xsl:comment>
                <title>
    ...
    <xsl:template name="documentGeneral">
        <table class="XSLtable">
            <tbody>
                <tr class="XSLtr">
                    <td class="XSLtd" width="20%" bgcolor="#00be96">
                        <span class="td_label">
                            <xsl:text>Document Id</xsl:text>
                        </span>
                    </td>
                    <td class="XSLtd" width="80%">
                            ...
                            <tr class="XSLtr">
                                <xsl:choose>
                                    <xsl:when test="n1:code">
                                        ...
                                        <td class="XSLtd" width="30%">
                                            <xsl:call-template name="show-id">
                                                <xsl:with-param name="id" select="n1:id" />
                                            </xsl:call-template>
                                        </td>
                                        <td class="XSLtd" width="15%" bgcolor="#00be96">
                                            <span class="td_label">
                        ...
                        <xsl:if test="n1:serviceEvent/@classCode and n1:serviceEvent/n1:code">
                            ...
                            <xsl:if test="$displayName">
                                <tr class="XSLtr">
                                    ...
                                    <td class="XSLtd" width="80%" colspan="3">
                                        <xsl:call-template name="show-code">
    ...
    <!-- RenderMultiMedia this currently only handles GIF's and JPEG's. It could, however, be extended by including other 
        image MIME types in the predicate and/or by generating <object> or <applet> tag with the correct params depending on the 
    ...
    <xsl:template name="show-name">
        ...
        <xsl:choose>
            <xsl:when test="$name/n1:family">
                ...
                <!--xsl:if test="$name/n1:suffix"> <xsl:text>, </xsl:text> <xsl:value-of select="$name/n1:suffix"/> </xsl:if -->
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template name="show-code">
        ...
        <xsl:choose>
            ...
            <!-- <xsl:when test="$the-valuesets/*/voc:system[@root=$this-codeSystem]/voc:code[@value=$this-code]/@displayName"> 
                <xsl:value-of select="$the-valuesets/*/voc:system[@root=$this-codeSystem]/voc:code[@value=$this-code]/@displayName"/> </xsl:when> -->
            <xsl:otherwise>
