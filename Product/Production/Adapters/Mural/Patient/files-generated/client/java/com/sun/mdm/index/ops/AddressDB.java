...
public final class AddressDB 
    ...
    public void create(Connection conn, HashMap opsmap, String[] keys, ObjectNode node) 
        throws OPSException {
        if (keys == null) {
            throw new OPSException ("AddressDB: invalid parent key(s) in create()");
        }
		...
		try {
	    	...
	    	int count = 1;
                if (address_obj.getAddressId() == null) {
                    address_obj.setAddressId(CUIDManager.getNextUID(conn, "ADDRESS"));
                }
    ...
    public ArrayList get(Connection conn, HashMap opsmap, String[] keys) throws OPSException {
        ...
        if (keys == null) {
            throw new OPSException ("AddressDB: invalid parent key(s) in get()");
        }
    ...
    {
        if (keys == null) {
            throw new OPSException ("AddressDB: invalid parent key(s) in update()");
        }
