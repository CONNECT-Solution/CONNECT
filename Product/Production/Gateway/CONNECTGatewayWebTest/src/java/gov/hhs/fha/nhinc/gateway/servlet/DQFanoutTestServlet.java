...
/**
 * Test Generator for DQ fanout test
...
public class DQFanoutTestServlet extends HttpServlet{
    ...
    public void doGet(HttpServletRequest req, HttpServletResponse response){
        ...
        if("true".equalsIgnoreCase(req.getParameter("rundqtest"))){
            ...
            runTest = true;
            validRequest = true;
            log.debug("DQFanoutTestServlet rundqtest has requestCount=" + requestCountStr
                    + " and targethcid=" + targethcid);
        }
        ...
        if(validRequest){
            try{
                ...
                if(runTest){
                    patientid = "123456789";
                    AdhocQueryRequest dq = (new AdhocQueryRequestGenerator()).generateTestRequest(
        ...
        try{
            ...
            TaskExecutor<QualifiedSubjectIdentifierType, AdhocQueryRequest, AdhocQueryResponse> dqexecutor =
                    new TaskExecutor<QualifiedSubjectIdentifierType, AdhocQueryRequest, AdhocQueryResponse>(
                        ExecutorServiceHelper.getInstance().checkExecutorTaskIsLarge(subjectList.size()) ? 
                            InitServlet.getLargeJobExecutorService() : InitServlet.getExecutorService(),
                        dqprocessor, dqclient, subjectList, dq, transactionId);
