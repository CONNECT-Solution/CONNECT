...
public class NhinXDRImpl
{
    ...
    private RegistryResponseType forwardToAgency(ProvideAndRegisterDocumentSetRequestType body,WebServiceContext context)
    {
        ...
        if (retryCount > 0 && retryDelay > 0 && exceptionText != null && !exceptionText.equalsIgnoreCase("")) {
                ...
                try {
                    ...
                    if (flag == 1) {
                        ...
                        try {
                            ...
                            log.error("Thread Got Interrupted while waiting on AdapterXDRSecuredService call :" + iEx);
                        } catch (IllegalArgumentException iaEx) {
                            log.error("Thread Got Interrupted while waiting on AdapterXDRSecuredService call :" + iaEx);
                        }
                        retryDelay = retryDelay + retryDelay; //This is a requirement from Customer
                    } else {
                        log.error("Unable to call AdapterXDRSecuredService Webservice due to  : " + e);
                        throw e;
                    }
                }
            }
            ...
            if (i > retryCount) {
                log.error("Unable to call AdapterXDRSecuredService Webservice due to  : " + catchExp);
                throw catchExp;
            }
