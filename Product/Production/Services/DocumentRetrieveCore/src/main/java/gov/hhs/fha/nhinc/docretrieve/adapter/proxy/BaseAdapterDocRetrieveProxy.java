...
public abstract class BaseAdapterDocRetrieveProxy implements AdapterDocRetrieveProxy {
     ...
     */
    String getEndPointFromConnectionManagerByAdapterAPILevel(AssertionType assertion) throws ConnectionManagerException {
        String url = null;
        ...
        if ((assertion != null) && (assertion.getImplementsSpecVersion() != null)) {
            ...
            if (assertion.getImplementsSpecVersion().equals(NhincConstants.UDDI_SPEC_VERSION.SPEC_2_0.toString())) {
                ...
                url = getoProxyHelper().getEndPointFromConnectionManagerByAdapterAPILevel(NhincConstants.ADAPTER_DOC_RETRIEVE_SERVICE_NAME, NhincConstants.ADAPTER_API_LEVEL.LEVEL_a0);
                if (NullChecker.isNullish(url)) {
                    url = getoProxyHelper().getEndPointFromConnectionManagerByAdapterAPILevel(NhincConstants.ADAPTER_DOC_RETRIEVE_SERVICE_NAME, NhincConstants.ADAPTER_API_LEVEL.LEVEL_a1);
                }
            ...
            url = getoProxyHelper().getAdapterEndPointFromConnectionManager(
                NhincConstants.ADAPTER_DOC_RETRIEVE_SERVICE_NAME);
        }
        return url;
    }
