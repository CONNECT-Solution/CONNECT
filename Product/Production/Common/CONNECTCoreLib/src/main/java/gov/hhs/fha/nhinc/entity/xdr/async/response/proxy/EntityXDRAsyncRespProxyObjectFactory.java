 ...
 * <bean id="entityxdrasyncresp" class="gov.hhs.fha.nhinc.entity.xdr.async.response.proxy.EntityXDRAsyncRespNoOpImpl"/>
 * the bean id is "entityxdrasyncresp" and an object of this type can be retrieved from
 * the application context by calling the getBean method like:
 * context.getBean("entityxdrasyncresp");. This returns an object that can be casted to
 * the appropriate interface and then used in the application code. See the
 * getEntityXDRAsyncRespProxy() method in this class.
 *
 * @author Jon Hoppesch
 */
public class EntityXDRAsyncRespProxyObjectFactory {
     ...
     * identifier of "entityxdrasyncresp."
     *
     * @return EntityXDRAsyncRespProxy instance
     */
    public EntityXDRAsyncRespProxy getEntityXDRAsyncRespProxy() {
        EntityXDRAsyncRespProxy proxy = null;
        if (context != null) {
            proxy = (EntityXDRAsyncRespProxy) context.getBean(BEAN_NAME);
        }
        return proxy;
    }
