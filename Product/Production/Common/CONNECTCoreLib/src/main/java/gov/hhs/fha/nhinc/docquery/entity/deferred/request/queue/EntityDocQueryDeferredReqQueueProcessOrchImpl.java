 ...
 */
import gov.hhs.fha.nhinc.docquery.entity.deferred.request.queue.proxy.EntityDocQueryDeferredRequestQueueProxyJavaImpl;
import gov.hhs.healthit.nhin.DocQueryAcknowledgementType;

public class EntityDocQueryDeferredReqQueueProcessOrchImpl {

    private static Log log = LogFactory.getLog(EntityDocQueryDeferredReqQueueProcessOrchImpl.class);

    public EntityDocQueryDeferredReqQueueProcessOrchImpl() {
    }
        ...
        try {
            ...
            if (respondingGatewayCrossGatewayQueryRequestType != null) {
                ...
                if (senderTargetCommunityId != null) {
                    ...
                    ack = entityDocQueryDeferredReqQueueProxyJavaImpl.respondingGatewayCrossGatewayQuery(respondingGatewayCrossGatewayQueryRequestType.getAdhocQueryRequest(), respondingGatewayCrossGatewayQueryRequestType.getAssertion(), targetCommunities);
    ...
    private RespondingGatewayCrossGatewayQueryRequestType extractRespondingGatewayQueryRequestType(Blob msgData) {
        log.debug("Begin EntityDocQueryDeferredReqQueueProcessOrchImpl.extractRespondingGatewayQueryRequestType()..");
        RespondingGatewayCrossGatewayQueryRequestType respondingGatewayCrossGatewayQueryRequestType = new RespondingGatewayCrossGatewayQueryRequestType();
        try {
            byte[] msgBytes = null;
            if (msgData != null) {
                ...
                log.debug("End EntityDocQueryDeferredReqQueueProcessOrchImpl.extractRespondingGatewayQueryRequestType()..");
            }
        } catch (Exception e) {
