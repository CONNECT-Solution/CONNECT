...
public class CommandLineFixture extends ActionFixture {
  ...
  public void contains() throws Exception {
    Parse processIdCell = cells.at(1);
    String[] lines = retrieveStreamContents(processIdCell);
    
    Parse targetCell = cells.at(2);
    String target = targetCell.text();
    if (contains(lines, target)) {
    	right(cells.at(2));
    } else {
    	wrong(cells.at(2));
    }
	...
	private String[] retrieveStreamContents(Parse processIdCell) {
		String [] lines = null;
	    ...
	    if (tokens != null) {
	      ...
	      if (streamId.equals("stdout")) {
	          lines = p.stdoutGatherer.getLines();
	        } else if (streamId.equals("stderr")) {
	          lines = p.stderrGatherer.getLines();
	        } else
	          streamExpressionError("Stream id not 'stdout' or 'stderr'"); 
	    } 
		return lines;
  ...
  private Boolean contains(String[] lines, String target) throws Exception {
