...
public final class PatientDB 
    ...
    public void create(Connection conn, HashMap opsmap, String[] keys, ObjectNode node) 
        throws OPSException {
        if (keys == null) {
            throw new OPSException ("PatientDB: invalid parent key(s) in create()");
        }
		...
		try {
	    	...
	    	int count = 1;
                if (patient_obj.getPatientId() == null) {
                    patient_obj.setPatientId(CUIDManager.getNextUID(conn, "PATIENT"));
                }
			...
			if (null != alias_list)
			{
				AliasDB alias = (AliasDB)opsmap.get("AliasDB");
				for(int i=0; i<alias_list.size(); i++)
				{
				   	AliasObject alias_obj = (AliasObject)alias_list.get(i);
                                        if (patient_obj.getPatientId() == null) {
                                            alias_obj.setAliasId(com.sun.mdm.index.idgen.CUIDManager.getNextUID(conn, "ALIAS"));
                                        }
			...
			{
				AddressDB address = (AddressDB)opsmap.get("AddressDB");
				for(int i=0; i<address_list.size(); i++)
				{
				   	AddressObject address_obj = (AddressObject)address_list.get(i);
                                        if (patient_obj.getPatientId() == null) {
                                            address_obj.setAddressId(com.sun.mdm.index.idgen.CUIDManager.getNextUID(conn, "ADDRESS"));
                                        }
				   	String[] pid = new String[1];
				   	pid[0] = patient_obj.getPatientId();
			...
			if (null != phone_list)
			{
				PhoneDB phone = (PhoneDB)opsmap.get("PhoneDB");
				for(int i=0; i<phone_list.size(); i++)
				{
				   	PhoneObject phone_obj = (PhoneObject)phone_list.get(i);
                                        if (patient_obj.getPatientId() == null) {
                                            phone_obj.setPhoneId(com.sun.mdm.index.idgen.CUIDManager.getNextUID(conn, "PHONE"));
                                        }
				   	String[] pid = new String[1];
				   	pid[0] = patient_obj.getPatientId();
    ...
    public ArrayList get(Connection conn, HashMap opsmap, String[] keys) throws OPSException {
        if (keys == null) {
            throw new OPSException ("PatientDB: invalid parent key(s) in get()");
        }
    ...
    {
        if (keys == null) {
            throw new OPSException ("PatientDB: invalid parent key(s) in update()");
        }
	    ...
	    catch (SQLException e)
	    {
            ...
            try
            {
                ...
                params = addobject(params, patient_obj.getCitizenship());
                params = addobject(params, patient_obj.getPatientId());
    	        log("    sbyntag:objname.makeClassName]Id: " + patient_obj.getPatientId());
