...
public final class AliasDB 
    ...
    public void create(Connection conn, HashMap opsmap, String[] keys, ObjectNode node) 
        throws OPSException {
        if (keys == null) {
            throw new OPSException ("AliasDB: invalid parent key(s) in create()");
        }
		...
		try {
	    	...
	    	int count = 1;
                if (alias_obj.getAliasId() == null) {
                    alias_obj.setAliasId(CUIDManager.getNextUID(conn, "ALIAS"));
                }
    ...
    public ArrayList get(Connection conn, HashMap opsmap, String[] keys) throws OPSException {
        ...
        if (keys == null) {
            throw new OPSException ("AliasDB: invalid parent key(s) in get()");
        }
    ...
    {
        if (keys == null) {
            throw new OPSException ("AliasDB: invalid parent key(s) in update()");
        }
