...
public final class PhoneDB 
    ...
    public void create(Connection conn, HashMap opsmap, String[] keys, ObjectNode node) 
        throws OPSException {
        if (keys == null) {
            throw new OPSException ("PhoneDB: invalid parent key(s) in create()");
        }
		...
		try {
	    	...
	    	int count = 1;
                if (phone_obj.getPhoneId() == null) {
                    phone_obj.setPhoneId(CUIDManager.getNextUID(conn, "PHONE"));
                }
    ...
    public ArrayList get(Connection conn, HashMap opsmap, String[] keys) throws OPSException {
        ...
        if (keys == null) {
            throw new OPSException ("PhoneDB: invalid parent key(s) in get()");
        }
    ...
    {
        if (keys == null) {
            throw new OPSException ("PhoneDB: invalid parent key(s) in update()");
        }
